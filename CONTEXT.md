...
See `docs/roadmap.md` for the manual sequence, UX decisions, invariants, and gates. Phases 3-8 are integrated: catalog-driven runtime data, versioned `FactoryDocument`/`BlueprintDocument` persistence with the local blueprint library (save/browse editor UI), independent blueprint insertion with named interfaces, command undo/redo across every mutating editor action, sidebar instance rows exposed to assistive technology as real selectable buttons, and a smoothly animated orientation-indicator arrow per instance that turns (and, for a rotated group, slides) instead of snapping instantly. Post-MVP work beyond this is not yet scheduled.
