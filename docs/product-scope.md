...
A interface egui já converte clique em coordenada do grid, usa o tile vazio como origem superior esquerda, cria IDs monotônicos e chama `FactoryLayout::place`. Sucesso desenha a instância e a adiciona a uma lista textual acessível; rejeições mostram feedback PT-BR sem consumir ID. Clicar em uma instância pintada ou em sua linha no sidebar seleciona-a e destaca seu footprint. Com seleção ativa, controles textuais ou setas movem uma tentativa de um tile; **Girar 90°** ou `R` chama `Rotation::clockwise()`. Ambos delegam à API de edição do domínio e preservam layout, seleção e alocador em erro. `Remover bloco`, `Delete` e `Backspace` solicitam confirmação antes de chamar `FactoryLayout::remove_instance`; cancelar, Escape ou o backdrop preservam layout, seleção e alocação de IDs. Preview, drag-and-drop, pan, zoom, undo/redo e persistência continuam pendentes na UI.
