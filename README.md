...
> **Current status:** Phase 4 is integrated. The default `eframe/egui` editor chooses one validated runtime `Catalog` at startup. That catalog drives base choices and bounds, the buildable palette, placement preview, painter, layout resolution, and semantic labels. The tracked public fallback supplies four confirmed bases and three confirmed buildables. A catalog can also declare products that capable buildables expose as a per-instance choice. The editor also saves and reopens the factory as a versioned local JSON document, and a player can save the current canvas selection as a named local blueprint and browse the local blueprint library from the sidebar. The next MVP work is independent blueprint insertion and exposed interfaces. The `iced` interface remains available only as a frozen legacy binary.
